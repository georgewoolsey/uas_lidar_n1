...
```{r, include=F, eval = F}
chm_rast <- terra::rast(
  cloud2trees:::search_dir_final_detected( file.path(tracking_df$path[1],"point_cloud_processing_delivery") )$chm_flist
)
```
...
timer_df %>% 
  ...
  kableExtra::column_spec(c(3,5,12), border_right = TRUE, include_thead = TRUE) %>% 
  kableExtra::collapse_rows(columns = c(1:2), valign = "top")
...
timer_df %>% 
  ggplot2::ggplot(
    mapping = aes(x = flight_agl, y = points_per_m2, color = factor(flight_mph))
  ) +
...
timer_df %>% 
  ggplot2::ggplot(
    mapping = aes(x = flight_agl, y = timer_total_time_mins_per_ha, color = factor(flight_mph))
  ) +
  ggplot2::geom_point(size = 4, alpha = 0.9) +
  ggplot2::scale_color_manual(
    values = viridis::rocket(n = 3, begin = 0.9, end = 0.5)[c(1,3)]
...
ggplot2::ggplot(
  preds_temp
  , mapping = ggplot2::aes(
    x = ref_tree_height_m, y = prob, color = as.factor(flight_agl)
  )
) +
  ggplot2::geom_line(size = 1.2) +
  ggplot2::geom_ribbon(ggplot2::aes(ymin = asymp.LCL, ymax = asymp.UCL, fill = as.factor(flight_agl)), alpha = 0.1, color = NA) +
  ggplot2::facet_grid(cols = dplyr::vars(flight_mph), labeller = label_both) +
  ggplot2::scale_y_continuous(labels = scales::percent) +
  ggplot2::labs(
    title = "Detection Probability Curves across 8 Flight Settings",
    subtitle = "Higher lines = Better detection. Steeper lines = Higher sensitivity to tree height.",
    x = "Tree Height (m)", y = "Probability of Detection", color = "Altitude", fill = "Altitude"
  ) +
  ggplot2::theme_light()
...
# ggplot2::ggplot(
#   preds_temp
#   , ggplot2::aes(x = ref_tree_height_m, y = prob, linetype = as.factor(flight_tf), color = as.factor(flight_tf))
# ) +
#   ggplot2::geom_line(size = 1) +
#   ggplot2::facet_wrap(~flight_agl + flight_mph, labeller = label_both) +
#   ggplot2::labs(
#     title = "Impact of Terrain Follow on Detection Curves",
#     subtitle = "Comparing Mode 0 (Off) vs Mode 1 (On) within specific Alt/MPH cells",
#     x = "Tree Height (m)", y = "Prob(Detected)", color = "Terrain Follow", linetype = "Terrain Follow"
#   ) +
#   ggplot2::theme_light()
# 
# find the height where probability first crosses 0.90
th_temp <-
  preds_temp %>%
  dplyr::group_by(flight_agl, flight_mph) %>%
  dplyr::filter(prob >= 0.70) %>%
  dplyr::slice_min(n = 1, order_by = ref_tree_height_m, with_ties = F) # keep only the smallest height that meets the criteria

ggplot2::ggplot(th_temp, ggplot2::aes(x = as.factor(flight_mph), y = ref_tree_height_m, fill = as.factor(flight_agl))) +
  ggplot2::geom_col(position = "dodge") +
  # ggplot2::facet_wrap(~flight_agl) +
  ggplot2::labs(
    # title = "Tree Height Threshold for 90% Detection"
    # subtitle = "Lower bars are better (means you can detect smaller trees with 90% certainty)"
    y = "Min Tree Height (m)"
    # , x = "Speed (mph)"
    # , fill = "Terrain Follow"
  ) +
...
fit_metric <- brm(
  formula = f_score ~ flight_agl * flight_mph * flight_tf,
  # formula = f_score ~ 0 + Intercept + flight_agl * flight_mph * flight_tf, # no intercept for faster centering?
  data = your_data,
...
# Ensure flight_agl and flight_mph are factors before running
your_data_factored <- your_data
your_data_factored$flight_agl <- as.factor(your_data_factored$flight_agl)
...
# main effects: compare levels of terrain follow
...
tf_diff <- fit_bayes %>%
  ...
  tidybayes::epred_draws(newdata = expand.grid(
    flight_tf = c("Level1", "Level2"),
    flight_agl = mean(your_data$flight_agl),
    flight_mph = mean(your_data$mph)
  )) %>%
  tidybayes::compare_levels(.epred, by = flight_tf)

# interaction effects: compare mph across altitude
# checking if the 'speed' effect is different at 200m vs 300m
int_diff <- fit_bayes %>%
  tidybayes::epred_draws(newdata = expand.grid(
    flight_agl = c(200, 300),
    flight_mph = c(20, 30),
    flight_tf = "Level1"
  )) %>%
  tidybayes::compare_levels(.epred, by = flight_mph) # compare speeds within each flight_agl

ggplot2::ggplot(int_diff, ggplot2::aes(x = .epred, y = as.factor(flight_agl))) +
