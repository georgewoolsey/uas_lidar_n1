...
```{r, include=FALSE, eval = F}
ground_truth_prediction_match_ans[[1]] %>% 
  dplyr::distinct(ref_match_idx) %>% 
  tidyr::drop_na() %>% 
  nrow()
validation_trees %>% nrow()

ground_truth_prediction_match_ans[[1]] %>% dplyr::filter(ref_match_idx==1) %>% dplyr::select(ref_tree_height_m, ref_dbh_cm)
validation_trees %>% dplyr::slice(1) %>% dplyr::select(field_tree_height_m, field_dbh_cm)
