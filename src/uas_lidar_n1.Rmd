...
agg_ground_truth_match_ans %>% 
  ...
  ggplot2::scale_color_viridis_d(option = "mako", end = 0.88, direction = -1) +
  ggplot2::labs(
...
```{r}
# stats::anova(mod_glm_temp, test = "Chisq") %>%
#   as.data.frame() %>%
#   kableExtra::kbl(caption = "sequential analysis of deviance") %>%
#   kableExtra::kable_styling()
# 'type = "response"' converts the log-odds back into 0-1 probabilities
...
# # table
# preds_temp %>%
#   dplyr::filter(ref_tree_height_m %in% c(5, 15, 25)) %>%
#   dplyr::select(ref_tree_height_m, flight_agl, flight_mph, prob, asymp.LCL, asymp.UCL) %>%
#   kableExtra::kbl(
#     caption = "detection probability by flight setting"
#     , digits = 3
#   ) %>%
#   kableExtra::kable_styling()
...
ggplot2::ggplot(preds_temp, ggplot2::aes(x = ref_tree_height_m, y = prob, linetype = as.factor(flight_tf), color = as.factor(flight_tf))) +
...
threshold_90 <- preds_temp %>%
  dplyr::group_by(flight_agl, flight_mph, flight_tf) %>%
  dplyr::filter(prob >= 0.90) %>%
  dplyr::slice(1) # Keep only the smallest height that meets the criteria

ggplot2::ggplot(threshold_90, ggplot2::aes(x = as.factor(mph), y = ref_tree_height_m, fill = as.factor(flight_tf))) +
  ...
  ggplot2::labs(
    ...
    x = "Speed (mph)", y = "Min Tree Height (m)", fill = "Terrain Follow"
  ) +
  ggplot2::theme_minimal()
...
tf_diff <- fit_bayes %>%
  ...
  tidybayes::epred_draws(newdata = expand.grid(
    flight_tf = c("Level1", "Level2"), # Replace with your actual levels
