...
tracking_df <- 
  ...
  dplyr::rename_with(.cols = -c(path,folder), .fn = ~paste0("flight_",.x,recycle0 = T) )
# huh?
...
chm_rast <- 
  ...
  purrr::map(
    \(x)
    terra::rast(
      file.path(tracking_df$path[x], "point_cloud_processing_delivery", "chm_0.25m.tif")
    )
  )
...
```{r, include = F, eval = F}
stand_boundary %>% sf::st_union() %>% sf::st_area() %>% as.numeric() %>% `/`(10000)
validation_plots <-
...
mapview::mapview(stand_boundary, color = "black", lwd = 2, alpha.regions = 0, label = F, legend = F, popup = F) + 
  mapview::mapview(validation_plots, zcol = "field_tree_height_m")
terra::plot(chm_rast[[2]])
stand_boundary %>% 
  sf::st_transform(terra::crs(chm_rast[[1]])) %>% 
  terra::vect() %>% 
  terra::plot(col = NA, border = "blue", add = T)
```
...
dtm_rast <- 
  ...
  purrr::map(
    \(x)
    terra::rast(
      file.path(tracking_df$path[x], "point_cloud_processing_delivery", "dtm_0.5m.tif")
    )
  )
