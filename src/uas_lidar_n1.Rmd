...
tracking_df %>% 
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_color_manual(values = c("gray44","gray11"), guide = "none") +
    ggplot2::theme_light() +
    ggplot2::theme(
...
the plot above is ordered by the total absolute percent error and maybe doesn't help us see the impact of different flight settings
...
now we'll look at the UAS bias in the original units for trees per hectare which indicates the over- or under-estimation
...
agg_ground_truth_match_ans %>% 
  ...
  tidyr::pivot_longer(cols = -c(dataset_factor, tidyselect::starts_with("flight_"))) %>% 
  dplyr::mutate(
    name = dplyr::recode_values(
      name
      , "diff_basal_area_m2_per_ha" ~ latex2exp::TeX("Basal Area $m^{2} \\cdot ha^{-1}$", output = "character")
  ...
  ggplot2::ggplot(
    mapping = ggplot2::aes(x = flight_mph, y = value, color = as.factor(flight_agl))
  ) +
  ggplot2::geom_point(size = 5,alpha=0.9) +
  ggplot2::geom_line(
    mapping = ggplot2::aes(group = flight_agl)
...
stand-level predictions for trees per hectare vary by flight setting but all flights resulted in TPH estimates near the field-measured data. The largest TPH error was a `r scales::percent(max(abs(agg_ground_truth_match_ans$pct_diff_trees_per_ha)),accuracy=.1)` underestimation while the best performing flight resulted in a `r scales::percent(min(abs(agg_ground_truth_match_ans$pct_diff_trees_per_ha)),accuracy=.1)` error (i.e. nearly identical to the reference value). The remaining flights achieved error rates between 1% and 4%. The UAS generally overpredicted the DBH of the largest trees. This bias propagated through to the stand-level basal area. As a result, basal area was overestimated across all flights. These overestimations ranged from 4.9% to 12.7% across the tested configurations.
