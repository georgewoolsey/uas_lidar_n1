...
timber_vol_df %>% 
  ...
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      y = dbh_in
      , x = ht_5_ft_bin
      , fill = merch_volume_ft3
    )
  ) +
  ggplot2::geom_tile(color = NA, width = 5, height = 1) +
  # ggplot2::geom_text(color = "white", size = 3) +
  ggplot2::facet_wrap(
    facets = dplyr::vars(species_symb)
    , ncol = 3
    # , scales = "free"
    , axes = "all"
  ) + 
  ggplot2::scale_x_continuous(expand = c(0, 0), breaks = scales::breaks_extended(n=15)) +
  ggplot2::scale_y_continuous(expand = c(0, 0), breaks = scales::breaks_extended(n=15)) +
...
Because volume estimation requires a species, we need to dig into what information we have on species for both the field-collected and UAS-predicted tree lists

### Field Data

the field data should have species?
