...
since our bayesian hierarchical model grouped by imputed FIA plot identifier that allows the intercept and the slope of height to vary by plot took quite a while to run for this relatively small area (~20-30 mins), let's try a random forest classifier to see if we can get the run time down. `cloud2trees` already implements random forest models to model missing HMD and CBH values that were not successfully extracted from the point cloud and uses a model averaging method that subsamples the training data (to reduce model fit times), estimates the model several times with different subsamples, then model averages the predictions. Because the species is not a metric variable that can averaged, we'll use the same method but employ a model majority voting system instead. let's do it with some of the `cloud2trees` built-in functionality.

random forest model majority voting for species with the objective to reduce estimation time while improving or maintaining the species composition accuracy compared to the field data
...
estimate_missing_max_n_training_temp <- 5555 # max FIA trees to use in each subsample
ntimes_temp <-
  ((nrow(treemap_tree_list_df)*0.5)/estimate_missing_max_n_training_temp) %>%
  ceiling() %>%
  max(22) %>%
  min(111)
# ntimes_temp
...
rf_spec_mod <- cloud2trees:::rf_subsample_and_model_n_times(
  predictors = treemap_tree_list_df %>% dplyr::select(tree_height_m,x,y)
  , response = treemap_tree_list_df$species_symbol
  , mod_n_subsample = estimate_missing_max_n_training_temp
...
### model majority vote
# cloud2trees:::rf_model_avg_predictions() was not built for non-numeric y-vars
# cloud2trees:::rf_model_avg_predictions(
#   mod_list = rf_spec_mod
#   , predict_df = predicted_trees[[1]] %>%
#     sf::st_drop_geometry() %>%
#     dplyr::select(tree_height_m,x,y)
# )
...
predicted_temp <- rf_model_vote_predictions(
  mod_list = rf_spec_mod
  , predict_df = 
    # predicted_trees[[1]] %>%
    xxx_temp %>% 
...
# predicted_trees[[1]] %>%
xxx_temp %>% 
  dplyr::mutate(species_symbol = predicted_temp$predicted) %>% 
  sf::st_drop_geometry() %>% 
  dplyr::count(species_symbol)
