...
```{r}
stand_boundary <- sf::st_read("c:/data/usfs/uas_sfm_tree_detection/data/field_validation/ref_data_Boundary.shp", quiet = T) %>% 
...
```{r}
reference_prediction_match <- function(variables) {
  if(
    is.na(clump_dist_m) || 
...
pred_data <- predicted_trees[[2]] %>% 
  dplyr::arrange(desc(tree_height_m)) #### should sort first
ref_data <- validation_trees %>% 
  dplyr::arrange(desc(field_tree_height_m)) #### should sort first
max_dist_m <- 3
...
# !!!!!!!! testing
# ferp <- 1
# cyl_pts <- nn_ans_temp$nn.idx[ferp, ]
# (cyl_pts <- cyl_pts[cyl_pts > 0] )
# pred_match_temp <- c()
# (cyl_pts <- cyl_pts[!(cyl_pts %in% pred_match_temp)] )
# 
# ggplot() + 
#   geom_sf(data = ref_data %>% dplyr::slice(ferp), color = "cyan") +
#   geom_sf(data = ref_data %>% dplyr::slice(ferp) %>% sf::st_buffer(3), color = "cyan", fill = NA) +
#   geom_sf(data = pred_data %>% 
#             sf::st_intersection(
#               ref_data %>% dplyr::slice(ferp) %>% sf::st_buffer(3)
#             )
#             , color = "black", size = 2.5) +
#   geom_sf(data = pred_data %>% dplyr::slice(cyl_pts), color = "magenta", size = 1) +
#   theme_void()
# 
# # get heights
# # ref_data %>% dplyr::select(field_tree_id, field_tree_height_m) %>% dplyr::glimpse()
# # ref_data$field_tree_height_m[ferp]
# # pred_data$tree_height_m[cyl_pts]
# 
# (ht_diffs <- abs(ref_data$field_tree_height_m[ferp]-pred_data$tree_height_m[cyl_pts]) )
...
# (cyl_idx <- which(ht_diffs == min_val))
# (match_idx <- cyl_pts[cyl_idx])

# iterate through the results to make matches
# add pred match tracker
ref_data$pred_match_idx <- 0
ref_data$min_height_diff_m <- as.numeric(NA)
for(i in 1:nrow(ref_coords_temp)){
  ...
  if (dplyr::coalesce(length(cyl_pts),0) < 1) {
    ref_data$min_height_diff_m[i] <- as.numeric(NA)
  }else{
    # get heights
    # ref_data %>% dplyr::select(field_tree_id, field_tree_height_m) %>% dplyr::glimpse()
    # ref_data$field_tree_height_m[ferp]
    ...
    if(is.infinite(min_val)){
      ...
      # update match
      ref_data$min_height_diff_m[i] <- min_val
...
ref_data %>% 
  dplyr::select(field_tree_id, field_tree_height_m, field_dbh_cm, pred_match_idx, min_height_diff_m) %>% 
  dplyr::left_join(
    pred_data %>% 
      sf::st_drop_geometry() %>% 
