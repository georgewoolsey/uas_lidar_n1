...
If the `RForInvt` [package](https://github.com/jstrunk001/RForInvt) was not broken at the time of analysis, I would have use it. Sadly, it was broken and I didn't want to try to fix it (but [see here](https://github.com/jstrunk001/RForInvt/issues/1)?) so....
...
validation_trees <- 
  ...
  dplyr::mutate(
    ...
    , species_symbol = toupper(spp) %>% stringr::str_squish() %>% stringr::word()
  ) %>% 
  # add on preds
  dplyr::left_join(
    vol_lookup_pred
...
treemap_tree_list_df <- 
  ...
  dplyr::mutate(
    ...
    , species_symbol = as.factor(species_symbol)
    , tm_id = as.factor(tm_id)
  )
# treemap_tree_list_df %>% dplyr::glimpse()
...
```{r}
pal_temp <- 
  length(unique(treemap_tree_list_df$species_symbol)) %>% 
  # RColorBrewer::brewer.pal(name = "Set2") %>% 
  viridis::turbo() %>% 
  setNames(
    as.character(unique(treemap_tree_list_df$species_symbol))
  )
# x,y
plt1_temp <-
  treemap_tree_list_df %>% 
  ...
  ggplot2::scale_color_manual(values = pal_temp) +
  ggplot2::coord_equal() + 
  ggplot2::labs(
    x = "x", y = "y"
  ) + 
...
# # #  Elapsed Time: 1500.86 seconds (Warm-up)
# # #                1839.83 seconds (Sampling)
# # #                3340.68 seconds (Total)
```

*9.3 hours later...*

this Softmax model with tree-level data was super slowwwwww and won't work for integration within `cloud2trees`
...
```{r, include=FALSE, eval=FALSE}
# let's try to bin the heights in ~~1~~ 2 m increments and then count the number of trees by plot, x, y, height bin, and species and then use the `weights()` in the `brms::brm()` formula
...
treemap_trees_df <- 
  treemap_trees_df %>% 
  dplyr::mutate(
    ht_bin = round(tree_height_m/ht_bin_size_temp)*ht_bin_size_temp
    , ht_bin_z = my_scale_fn(ht_bin, train_stats_temp$h_mean, train_stats_temp$h_sd)
  )
# treemap_trees_df %>% dplyr::glimpse()
...
# # #                1048.05 seconds (Total) ... tm_id as character
# # #                2136.31 seconds (Total) ... non-norm ht, tm_id factor
# # #                2163.11 seconds (Total) ... norm ht, tm_id factor
```

let's compare the model estimates

```{r}
pred_i_mod_temp <- predict(brms_spec_i_mod, newdata = predicted_trees[[1]])
pred_h_mod_temp <- predict(brms_spec_h_mod, newdata = predicted_trees[[1]])
```

huh?

```{r}
pred_h_mod_temp %>% dimnames()
pred_h_mod_temp %>% dplyr::glimpse()
# find index of max column for each row
max_col_idx_temp <- max.col(pred_h_mod_temp, ties.method = "first")
# get the actual names using the indices
max_col_names <- colnames(pred_h_mod_temp)[max_col_idx_temp]
max_col_names %>% table()
```
