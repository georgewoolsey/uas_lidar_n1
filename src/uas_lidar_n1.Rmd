...
We'll evaluate stand level aggregation accuracy for stand basal area and trees per hectare. For these stand metrics, we calculated the difference ($predicted - reference$) to measure the bias in the original units (e.g., $\frac{m^{2}}{ha}$ for basal area) which indicates the over- or under-estimation of the forest structure. We also calculated the percent difference ($\frac{predicted-reference}{reference}$) to measure the relative bias (scale-independent) to capture the magnitude of the error.
...
ggplot2::ggplot(df, ggplot2::aes(x = pct_diff, y = stats::reorder(flight_id, pct_diff))) +
  ggplot2::geom_point(size = 3) +
  ggplot2::geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  ggplot2::labs(x = "Percent Difference (%)", y = "Flight ID")

ggplot2::ggplot(df, ggplot2::aes(x = flight_id, y = total_diff, color = pct_diff)) +
  ggplot2::geom_segment(ggplot2::aes(x = flight_id, xend = flight_id, y = 0, yend = total_diff)) +
  ggplot2::geom_point(size = 4) +
  ggplot2::scale_color_viridis_c()

ggplot2::ggplot(df, ggplot2::aes(x = tph_pct_diff, y = sba_pct_diff, label = flight_id)) +
  ggplot2::geom_point() +
...
# Step 2: Plot
ggplot2::ggplot(plot_data, ggplot2::aes(x = Metric, y = Value, group = flight_id)) +
  # Draw the connecting lines
  ggplot2::geom_line(ggplot2::aes(color = flight_id), size = 1) +
  # Add points at the ends
  ggplot2::geom_point(ggplot2::aes(color = flight_id), size = 3) +
  # Add text labels for each flight next to the lines
  ggplot2::geom_text(data = subset(plot_data, Metric == "SBA_pct"),
                     ggplot2::aes(label = flight_id), 
                     hjust = 1.2, size = 3) +
  # Aesthetic clean up
  ggplot2::theme_minimal() +
  ggplot2::labs(title = "Flight Consistency: SBA vs TPH Percent Difference",
                y = "Percent Difference (%)",
                x = "") +
  ggplot2::geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5)

ggplot2::ggplot(flights) +
  # Draw the bar between the two points
  ggplot2::geom_segment(ggplot2::aes(x = Ref_SBA, xend = Pred_SBA, 
                                     y = flight_id, yend = flight_id),
                        color = "grey70", size = 1.5) +
  # Draw the Reference point
  ggplot2::geom_point(ggplot2::aes(x = Ref_SBA, y = flight_id, color = "Reference"), 
                      size = 4) +
  # Draw the Predicted point
  ggplot2::geom_point(ggplot2::aes(x = Pred_SBA, y = flight_id, color = "Predicted"), 
                      size = 4) +
  # Styling
  ggplot2::scale_color_manual(values = c("Reference" = "black", "Predicted" = "royalblue")) +
  ggplot2::theme_minimal() +
  ggplot2::labs(title = "Absolute Difference: Predicted vs Reference SBA",
                x = "Basal Area (m2/ha)",
                y = "Flight ID",
                color = "Value Type")
