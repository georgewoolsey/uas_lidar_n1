...
processing time was much better and took only `r scales::comma(mins_temp, accuracy = 0.01)` minutes (`r scales::comma(mins_temp*60, accuracy = 0.01)` seconds) to estimate `r ntimes_temp` RF models using `r scales::comma(mod_n_subsample_temp)` training records (`rscales::percent(mod_n_subsample_temp/nrow(treemap_tree_list_df), accuracy = 0.01)` of the training data) from which will use majority voting to determine the predicted class
...
look at the  Bray-Curtis **dissimilarity** measure and Schoener's D (total overlap) index which we looked at above is a **similarity** measure
