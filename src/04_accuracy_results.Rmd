...
dta_temp <- 
  ...
  purrr::map(
    ...
    x %>% 
      ...
      dplyr::mutate(
        tree_dbh_cm_bin = 
          cut(ref_dbh_cm, breaks = c(0,ostory_dbh_temp,ceiling(max(validation_trees$field_dbh_cm))), right = F) %>% 
          # ggplot2::cut_width(ref_dbh_cm, width = ostory_dbh_temp, boundary = 0, closed = "left")
