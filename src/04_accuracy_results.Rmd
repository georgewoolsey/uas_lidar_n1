...
let's check this for overstory versus understory trees. we'll use 10 cm (`r scales::comma(10/2.54,accuracy=0.1,suffix = " in")`) DBH as a threshold for classifying overstory and understory since it is a common value used in regional forest inventories.

```{r, fig.height=10}
# 10 cm DBH, a common regional forest inventory breakpoint for describing overstory and understory forest densities
ostory_dbh <- 10
# plt
dta_temp <- 
  ground_truth_prediction_match_ans %>%
...
let's shift our focus to relative error (MAPE) while providing absolute context (RMSE and ME) as text or secondary metrics to look into the penalty we face due to inaccuracies in DBH for small stems. for example, a 3 cm error on a 6 cm small tree is a 50% relative error while that same 3 cm error on a 30 cm overstory tree is a minor 10% error. we'll use 10 cm (`r scales::comma(10/2.54,accuracy=0.1,suffix = " in")`) DBH as a threshold for classifying overstory and understory since it is a common value used in regional forest inventories.
...
p_temp
ggplot2::ggsave("../data/dbh_acc_by_size.jpg",p_temp+ggplot2::theme(plot.subtitle = ggplot2::element_blank()), height = 10, width = 8)
```
...
p_temp
ggplot2::ggsave("../data/ht_acc_by_size.jpg",p_temp+ggplot2::theme(plot.subtitle = ggplot2::element_blank()), height = 10, width = 8)
```
...
ggplot2::ggplot(
  ...
  ggplot2::geom_line(lwd = 1.2, alpha = 0.85) +
  ggplot2::facet_grid(
    cols = dplyr::vars(flight_altitude_m)
