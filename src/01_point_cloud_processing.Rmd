...
let's make a custom linear function that expands the search window for shorter trees but keeps roughly the same window size for taller trees between 15-25 m (we don't expect many trees above this in our study area). 

$$
\text{Search Radius (m)} = \begin{cases} 
0.52 & \text{if } CHM < 1 \\ 
0.52 + 0.14(CHM) & \text{if } 1 \le CHM < 15 \\ 
1.12 + 0.10(CHM) & \text{if } 15 \le CHM \le 40 \\ 
5.12 & \text{if } CHM > 40 
\end{cases}
$$
...
my_linear <- function(x) {
  y <- dplyr::case_when(
    ...
    , x < 15 ~ 0.52 + x*0.14
    # next piece:
        # we fix the slope at 0.1 and need to find the intercept connecting with the lower piece
        # connect to (15,2.62)
        # b = 2.62 - (0.1*15) = 1.12
    , x <= 40 ~ 1.12 + x*0.1
    # cap once we get to 40 m
    , T ~ (1.12 + 40*0.1)
  )
  # safety
  y <- pmax(y, 0.52)
...
my_linear <- function(x) {
  y <- dplyr::case_when(
    ...
    , x < 15 ~ 0.44 + x*0.16
    # next piece:
        # we'll fix the slope at 0.08 and need to find the intercept connecting with the lower piece
        # at x=15 > y = 0.44 + 15*0.16 = 2.84
        # connect to (15,2.84)
        # b = 2.84 - (0.08*15) = 1.64
    , x <= 40 ~ 1.64 + x*0.08
    # cap once we get to 40 m
...
# my_linear
ggplot2::ggplot() +
  ggplot2::geom_function(fun = cloud2trees::itd_ws_functions()$lin_fn, ggplot2::aes(color = "lin_fn"), lwd = 1) +
  ggplot2::geom_function(fun = cloud2trees::itd_ws_functions()$exp_fn, ggplot2::aes(color = "exp_fn"), lwd = 1) +
  ggplot2::geom_function(fun = my_linear, ggplot2::aes(color = "my_linear"), lwd = 1) +
  # ggplot2::geom_function(fun = my_linear1, ggplot2::aes(color = "my_linear1"), lwd = 1) +
  # ggplot2::geom_function(fun = my_linear2, ggplot2::aes(color = "my_linear2"), lwd = 1) +
  ggplot2::xlim(-5,50) +
  ggplot2::labs(x = "heights", y = "ws", color = "") +
  ggplot2::theme_light()
```
...
```{r,eval=F}
ggplot2::ggplot() +
  ggplot2::geom_function(fun = itd_tuning_ans$ws_fn_list$lin_fn, ggplot2::aes(color = "lin_fn"), lwd = 1) +
  ggplot2::geom_function(fun = itd_tuning_ans$ws_fn_list$exp_fn, ggplot2::aes(color = "exp_fn"), lwd = 1) +
  ggplot2::geom_function(fun = my_linear, ggplot2::aes(color = "my_linear"), lwd = 1) +
  ggplot2::xlim(-5,40) +
  ggplot2::labs(x = "heights", y = "ws", color = "") +
  ggplot2::theme_light()
