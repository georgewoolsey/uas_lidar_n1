...
This section is dedicated to analysis that was performed during evaluation but was not discussed in the manuscript for publication including:

* [Volume Estimation and Accuracy Evaluation](#vol_est)
* [Tree-Level Species Predictions](#species_pred)

## Volume Estimates {#vol_est}
...
ground_truth_prediction_match_ans_updt <-
  ...
  purrr::map(function(x){
    ...
    training_df <-
      readr::read_csv(
        file.path(
          tracking_df %>% dplyr::filter(dataset_factor==x) %>% dplyr::pull(path)
          , "point_cloud_processing_delivery"
          , "regional_dbh_height_model_training_data.csv"
        )
        , show_col_types = F
    ...
    pred_ids <-
      ...
      purrr::discard(~.x==0) # gets rid of FN records
    set.seed(33)
    pred_spec_df <-
      dplyr::tibble(
    ...
    new_ground_truth_prediction_match_ans <- 
    ...
    return(new_ground_truth_prediction_match_ans)
  })
names(ground_truth_prediction_match_ans_updt) <- tracking_df$dataset_factor
```
...
```{r}
# agg_ground_truth_match_ans$dataset_factor %>% class()
agg_ground_truth_match_ans <-
  agg_ground_truth_match_ans %>% 
  dplyr::left_join(
    ground_truth_prediction_match_ans_updt %>%
...
```{r,eval=T, fig.height = 10.5, fig.width = 8.7}
# x,y
plt1_temp <-
  treemap_tree_list_df %>% 
...
# scales::comma(timer_temp$timer_brms_spec_h_mod_mins,accuracy=0.1)
```
...
```{r, fig.height = 9.8, fig.width = 6.5}
predicted_trees %>%
  purrr::list_rbind(names_to = "dataset_factor") %>%
  dplyr::mutate(
    dataset_factor = ordered(dataset_factor, levels(tracking_df$dataset_factor))
  ...
  dplyr::arrange(dataset_factor, species_symbol) %>% 
  # plot it
  # plt
  ggplot2::ggplot() +
    ggplot2::geom_sf(
...
# ggplot2::ggsave(filename = "../data/spec_predicted.jpg", height = 9.8, width = 6.5, dpi = "print")
```
...
dta_temp %>% 
  ...
  dplyr::rename(
    `altitude (m)` = flight_altitude_m
    , `speed (m/s)` = flight_meters_per_second
  ) %>% 
  # dplyr::glimpse()
  kableExtra::kbl(
...
predicted_trees %>%
  ...
  dplyr::slice_sample(prop = 0.44, by = dataset_factor) %>% 
  dplyr::inner_join(
    tracking_df %>% 
      dplyr::select(-c(path))
    , by = "dataset_factor"
